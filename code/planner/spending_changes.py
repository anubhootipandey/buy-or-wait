            ...
            if min_allowed is not None and min_allowed < original_amount:
                actions.append(
                    SpendingChangeAction(
                        ...
                        severity=0,
                        # id(series): the exact physical series this action
                        # came from - see SpendingChangeAction's docstring.
...
def _ambiguous_series_keys(recurring_series: list[RecurringSeries]) -> frozenset[tuple]:
    """(category, event_type, direction) keys claimed by MORE THAN ONE
    detected series - the Rule 9 alternating-series-split case. A bare
    key match is not a safe identity for any of these; see
    `apply_changes_to_forecast`."""
    counts: dict[tuple, int] = {}
    for s in recurring_series:
        key = (s.key.category, s.key.event_type, s.key.direction)
        counts[key] = counts.get(key, 0) + 1
    return frozenset(key for key, n in counts.items() if n > 1)
...
def apply_changes_to_forecast(
    ...
    `recurring`) checkpoint matching one of `changes`' TARGETED series
    adjusted to that change's amount, and the balance walk redone from
    scratch. Historical `settled` checkpoints (today's already-committed
    cash) are never touched - only forward-looking events can be changed.

    Series identity, not just (category, event_type, direction)
    -------------------------------------------------------------
    A generated (`source == "recurring"`) checkpoint always carries the
    `series_instance_id` of the exact `RecurringSeries` object that
    produced it (see `CashEvent.series_instance_id`). Matching on that
    identity - never on the bare key alone - is what keeps two Rule-9
    split halves (which share one key but are different series) from
    ever contaminating each other: a change built for one split half can
    now only ever match occurrences generated by that same series object.

    A `known_future` checkpoint has no series object to match by identity
    (it is a real, already-scheduled dataset row, not something
    `project_series` generated) - it is matched by the bare key, exactly
    as before, but ONLY when that key is unambiguous (claimed by exactly
    one detected series). When the key is ambiguous (a split pair),
    matching a known_future event to "the" targeted series would be a
    guess - Stage 4c's own `match_series_for_fact` refuses the identical
    guess for evidence facts, and this does the same rather than risk
    applying one split half's change to a known-future occurrence that
    might belong to the other half.
    """
    changes_by_instance = {c.series_instance_id: c for c in changes}
    changes_by_key = {c.series_key: c for c in changes}
    ambiguous_keys = _ambiguous_series_keys(forecast.recurring_series)
    ...
    for cp in forecast.checkpoints:
        ...
        if event.source == "recurring":
            change = changes_by_instance.get(event.series_instance_id)
        elif event.source == "known_future":
